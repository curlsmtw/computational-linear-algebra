...
def compute_determinant(matrix):
    P, L, U = lg.lu(matrix)
    det_A_LU = np.prod(np.diag(U)) * np.linalg.det(P)
    det_A_numpy = np.linalg.det(matrix)
    return det_A_LU, det_A_numpy

if __name__ == "__main__":
    matrix_six = np.array([[4,-1,-1], [-1,4,-1], [-1,-1,4]])
    det_A_LU, det_A_numpy = compute_determinant(matrix_six)
    
    print("Original Matrix:")
    print(matrix_six)
    print("Determinant using LU decomposition:", det_A_LU)
    print("Determinant using numpy's built-in function:", det_A_numpy)